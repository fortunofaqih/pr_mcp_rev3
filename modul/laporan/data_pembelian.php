...
// 2. QUERY STRING (sertakan filter_petugas)
$query_string = "tgl_min=$tgl_min&tgl_max=$tgl_max&keyword=$keyword&id_user_beli=$filter_petugas";

// 3. AMBIL DAFTAR PETUGAS untuk dropdown (hanya yang punya data di pembelian)
...
<head>
    ...
    <style>
        ...
        /* Highlight filter petugas aktif */
        .select-petugas-aktif {
            border-color: #0000FF !important;
            box-shadow: 0 0 0 0.2rem rgba(0,0,255,.15) !important;
            font-weight: bold;
            color: #0000FF;
        }
...
foreach ($data_tampil as $row) {
    ...
    $tgl_format = ($row['tgl_beli_barang'] != '0000-00-00' && !empty($row['tgl_beli_barang']))
                  ? date('d/m/y', strtotime($row['tgl_beli_barang'])) : '-';
    $ket = $row['alokasi_stok'] ?? '';
    if (!empty($row['plat_nomor'])) $ket .= ' ' . $row['plat_nomor'];
    if (!empty($row['driver']))     $ket .= ' (' . $row['driver'] . ')';
    if (!empty($row['keterangan']) && $row['keterangan'] != '-') $ket .= ' | ' . $row['keterangan'];
    $print_rows[] = [
        'tgl'      => $tgl_format,
...
$(document).ready(function() {
    ...
    $('.hitung').on('input', function() {
        ...
        if (qty > 0) $('#edit_harga').val(Math.round(total / qty));
    });
});
...
function cetakLaporan() {
    ...
    // ── Konfigurasi ───────────────────────────────────────────
    var ROWS_PER_PAGE = 18;   // baris data per halaman (sesuaikan jika perlu)

    // ── Nomor form: PB-YYYYMM-NNN → suffix huruf per sheet ──
    var now       = new Date();
    var yymm      = now.getFullYear() + '' + String(now.getMonth()+1).padStart(2,'0');
    var seqNum    = String(Math.floor(Math.random()*900)+100); // 3 digit acak (ganti dgn counter DB jika ada)
    var baseForm  = 'PB-' + yymm + '-' + seqNum;             // mis. PB-202603-147
    var ALPHA     = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';

    // ── Pecah rows ke halaman ─────────────────────────────────
    var pages = [];
    ...
    + 'body { font-family:Arial,sans-serif; font-size:8pt; margin:0; padding:0; background:#fff; color:#000; }'

    /* setiap .page-block = 1 halaman cetak */
    ...
    + '  background:#eeeeee; border:0.5px solid #aaa; padding:2px 6px; margin-bottom:4px;'
    + '  font-size:7pt; -webkit-print-color-adjust:exact; print-color-adjust:exact; }'
    ...
    + 'table.data th { background-color:#333 !important; color:#fff !important;'
    + '  -webkit-print-color-adjust:exact; print-color-adjust:exact;'
    + '  text-align:center; font-size:7pt; font-weight:bold;'
    + '  border:0.5px solid #000; padding:3px 3px; }'
    + 'table.data td { border:0.5px solid #000; padding:2px 3px;'
    ...
    /* footer bawah setiap halaman */
    + '.page-footer { display:flex; justify-content:space-between; align-items:flex-end;'
    + '  margin-top:4px; padding-top:3px; border-top:1px solid #000; font-size:7.5pt; }'
    + '.total-box { background:#333; color:#fff; padding:3px 8px; font-size:8.5pt;'
    + '  font-weight:bold; -webkit-print-color-adjust:exact; print-color-adjust:exact; }'
    + '.ttd-box { text-align:center; font-size:7.5pt; }'
    + '.ttd-line { border-top:1px solid #000; width:130px; margin:20px auto 2px; }'

    /* nomor form pojok kanan bawah */
    + '.form-no { font-size:7pt; color:#333; font-weight:bold;'
    + '  border:0.5px solid #aaa; padding:2px 5px; background:#f9f9f9;'
    + '  -webkit-print-color-adjust:exact; print-color-adjust:exact; }'

    /* kontrol screen */
    + '.print-ctrl { text-align:center; padding:8px; margin:8px; background:#f8f9fa;'
    + '  border:1px solid #ddd; border-radius:4px; }'
    + '.print-ctrl button { border:none; padding:6px 16px; margin:0 4px; border-radius:3px;'
    ...
    + '.page-badge { display:inline-block; background:#555; color:#fff; font-size:7pt;'
    + '  padding:1px 5px; border-radius:2px; margin-left:6px;'
    + '  -webkit-print-color-adjust:exact; print-color-adjust:exact; }'
    + '@media print { .print-ctrl{display:none!important;} body{padding:0;margin:0;} }';

    // ── Bangun setiap halaman ─────────────────────────────────
    var allPages = '';
    pages.forEach(function(pageRows, pi) {
        var sheetLetter = ALPHA[pi] || String(pi+1);   // A, B, C … atau angka jika >26
        var formNo      = baseForm + '/' + sheetLetter; // PB-202603-147/A

        // Baris tabel halaman ini
        var tbody = '';
        pageRows.forEach(function(r, ri) {
            var globalIdx = pi * ROWS_PER_PAGE + ri + 1; // nomor urut global
            var bg = r.termurah
                ? 'background:#e8fff0;-webkit-print-color-adjust:exact;print-color-adjust:exact;'
                : '';
            tbody += '<tr style="' + bg + '">'
        ...
        // Footer: hanya halaman terakhir tampilkan grand total + TTD
        ...
        var ttdFoot = isLast
            ? '<div class="ttd-box">'
              + '<div style="font-size:7pt;color:#555;">Mengetahui,</div>'
        ...
        allPages +=
            ...
            + '<h4>LAPORAN REALISASI PEMBELIAN HARIAN</h4>'
            + '</div>'
            ...
            + '<th class="c-brg">NAMA BARANG</th>'
            + '<th class="c-qty">QTY</th>'
            + '<th class="c-hrg">HARGA</th>'
            ...
            + '<span class="form-no">Form No. ' + formNo + '</span>'
            + '</div>'
            + '</div>'
    ...
    + '<span style="font-size:8px;color:#888;margin-left:10px;">'
    + '&#128196; ' + totalPages + ' halaman &nbsp;|&nbsp; Kertas: 21.5&times;16.5 cm Landscape'
    + '</span>'
    + '</div>'
