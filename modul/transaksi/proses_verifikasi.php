 ...
 * Aksi: APPROVE → pindah ke tabel pembelian + update stok + status item TERBELI
 *       TOLAK   → hapus staging + status item kembali PENDING
 */
session_start();
...
try {
    ...
    if ($aksi === 'APPROVE') {
        // Ambil field yang sudah diedit admin gudang
        ...
        $id_brg_val = ($stg['id_barang'] > 0) ? $stg['id_barang'] : 'NULL';
        ...
        // 1. Insert ke tabel pembelian (final)
        ...
        // 3. Update status item → TERBELI
        if (!mysqli_query($koneksi, "UPDATE tr_request_detail SET status_item='TERBELI' WHERE id_detail = $id_detail")) {
            throw new Exception('Gagal update status item: '.mysqli_error($koneksi));
        }

        // 4. Cek sisa item PR → update status header
        $sisa = 1;
        if ($id_request > 0) {
        ...
        echo json_encode(['status'=>'ok','aksi'=>'APPROVE','message'=>'Data disetujui dan tersimpan ke buku realisasi.','pr_selesai'=>($sisa===0)]);

    } else {
        // TOLAK: kembalikan status item ke PENDING, hapus staging
        ...
        // 2. Cek sisa item yang masih MENUNGGU VERIFIKASI atau sudah TERBELI
        //    Jika masih ada yg proses, status PR tetap PROSES, jika tidak ada → kembali PENDING
        if ($id_request > 0) {
            $q_sisa = mysqli_query($koneksi, "SELECT id_detail FROM tr_request_detail
                                              WHERE id_request = $id_request
