<?php
// ============================================================
// cetak_po.php - Revisi Final Berdasarkan Database Master Supplier
// ============================================================
session_start();
...
if ($id_po > 0) {
    ...
    exit("<div style='padding:20px; font-family:sans-serif;'><h3>Error: PO tidak ditemukan.</h3></div>");
}

// Query disesuaikan dengan kolom tabel master_supplier Anda
...
$po = mysqli_fetch_array($query_po);

if (!$po) { 
    exit("<div style='padding:20px; font-family:sans-serif;'><h3>Data PO Belum Dibuat</h3></div>"); 
}

$pr = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM tr_request WHERE id_request = '".$po['id_request']."'"));
...
$tgl_po_fmt = date('d F Y', strtotime($po['tgl_po']));
$total_setelah_diskon = $po['subtotal'] - $po['diskon'];
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        .kop-info h1 { font-size: 18px; font-weight: 800; margin: 0; }
    ...
    <div class="po-page">
        <div class="kop-header">
            ...
            <div class="kop-info">
                ...
                <p>Jl. Raya Karang Pilang 33 </p>
                <p>Telp: (031) 7661121, 7661354 | Fax: (031) 7660910</p>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if($d['kwalifikasi']): ?><br><small style="color:#444;">Spec: <?= $d['kwalifikasi'] ?></small><?php endif; ?>
                        <?php if($d['keterangan']): ?><br><small style="color:#666;">Ket: <?= $d['keterangan'] ?></small><?php endif; ?>
                    </td>
                    <td class="text-center"><?= (float)$d['jumlah'] ?></td>
                    <td class="text-center"><?= $d['satuan'] ?></td>
                    <td class="text-end"><?= number_format($d['harga_satuan_estimasi'], 0, ',', '.') ?></td>
                    <td class="text-end"><?= number_format($d['subtotal_estimasi'], 0, ',', '.') ?></td>
                </tr>
                <?php endwhile; ?>
                ...
                <tr>
                    ...
                    <td class="text-end"><?= $po['diskon'] > 0 ? '- '.number_format($po['diskon'], 0, ',', '.') : '0' ?></td>
                </tr>
                ...
                <tr>
                    <td class="text-end fw-bold">PPN <?= $po['ppn_persen'] ?>%</td>
                    <td class="text-end"><?= number_format($po['ppn_nominal'], 0, ',', '.') ?></td>
                </tr>
                <tr style="background: #eee;">
                    <td class="text-end fw-bold" style="font-size: 12px;">GRAND TOTAL</td>
                    <td class="text-end fw-bold" style="font-size: 12px;">Rp <?= number_format($po['grand_total'], 0, ',', '.') ?></td>
                </tr>
            </tbody>
        </table>
