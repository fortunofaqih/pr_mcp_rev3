...
$role_valid = ['administrator', 'manager', 'admin_gudang', 'bagian_pembelian'];
...
if (mysqli_num_rows($query) === 1) {
    ...
    if (password_verify($password, $data['password'])) {
        ...
        if ($login_sebagai === $role_asli) {
            ...
            // Kasus khusus: user punya akses gudang tambahan (misal Bu Helena)
            $boleh_login = true;
        ...
        // ============================================================
        // SET SESSION
        // Gunakan $login_sebagai sebagai role aktif (bukan role asli)
        // sehingga tampilan dashboard menyesuaikan pilihan saat login
        // ============================================================
        $_SESSION['id_user']       = $data['id_user'];
        $_SESSION['username']      = $data['username'];
        $_SESSION['nama']          = $data['nama_lengkap'];
        $_SESSION['role']          = $login_sebagai;   // ← role yang DIPILIH saat login
        $_SESSION['role_asli']     = $role_asli;       // ← role asli di DB (untuk referensi)
